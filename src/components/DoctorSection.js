...
export default function DoctorSection({isHome}) {
  ...
  <div className=" bg-blue-100 ">
      ...
      <AvatarFallback className="font-semibold text-cyan-400">DAS</AvatarFallback>
      </Avatar>
      </div> 

    <CardTitle className="rounded-xl border bg-gradient-to-r from-violet-300 to-indigo-400 mx-auto p-2 my-2">{doctor.name}</CardTitle>
    <CardDescription className='my-2 font-semibold font-xl'> {doctor.category}</CardDescription>
  ...
  <CardContent >
    ...
    <div className="flex justify-between items-center mx-4 ">
     <span className="font-bold flex  items-center gap-2"><ClockIcon />Time</span>
      <h1>{doctor.appointment}</h1>
    </div>    
    <div className=" flex justify-between items-center mx-3 ">
      <span className="font-bold flex  items-center gap-2"> <HomeIcon /> Hospital  </span>
    <p className=" rounded-xl  p-2 font-semibold">  {doctor.hospital}</p>
  ...
  <CardFooter> 
    <Link href={`/doctors/${doctor.id}`}>
      ...
      <Button className=" rounded bg-gradient-to-r from-indigo-400 from-10% via-sky-300 via-50%" > Book Appointment</Button>
    </Link>
